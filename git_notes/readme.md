...
> 一个 Git 仓库里面又包含了其他 Git 仓库。

Git 会把它们识别为 **子模块（submodule）**。

因此 Git 记录的是：

```
bird-record = 某个仓库的某个 commit

uni-jiuji = 某个仓库的某个 commit
```
...
```
160000 commit xxxx bird-record

160000 commit xxxx uni-jiuji
```
...
```bash
git rm --cached bird-record

git rm --cached uni-jiuji
```

---

# 6. git rm --cached 是什么意思？
...
```
new file: bird-record/xxx

new file: uni-jiuji/xxx
```
...
```bash
git commit -m "fix: convert submodule to normal folder"
```
...
```bash
git ls-tree -r github/main --name-only
```
